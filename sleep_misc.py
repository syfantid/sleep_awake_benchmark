...
def rescore_models(df, models, tl_min_sleep=10, tl_min_awake=20):
    ...
    for model in models:
        ...
        df["tl_resc_" + model] = tudor_locke(df["tl_" + model], min_minutes_sleep=tl_min_sleep,
                                             min_minutes_awake=tl_min_awake)
        ...
        all_models.extend(["tl_" + model, "resc_" + model, "resc_tl_" + model, "tl_resc_" + model, model + "_max",
                           "tl_" + model + "_max", "resc_tl_" + model + "_max"])
...
def summary_table(results):
    ...
    for alg in results:
        ...
        values["SEWholeDF"].append(print_format % (
        100. * results[alg]["EfficiencyWholeDF"].mean(), 100. * results[alg]["EfficiencyWholeDF"].std()))
        values["SEGTBlock"].append(print_format % (
        100. * results[alg]["EfficiencyGTBlock"].mean(), 100. * results[alg]["EfficiencyGTBlock"].std()))
        values["SESelfBlock"].append(print_format % (
        100. * results[alg]["EfficiencySelfBlock"].mean(), 100. * results[alg]["EfficiencySelfBlock"].std()))
        values["SESelfBlock5Min"].append(print_format % (
        100. * results[alg]["EfficiencySelfBlock5min"].mean(), 100. * results[alg]["EfficiencySelfBlock5min"].std()))

        values["TotalSleep"].append(
            print_format % (results[alg]["TotalSleep"].mean(), results[alg]["TotalSleep"].std()))
        values["TotalSleepBlock"].append(
            print_format % (results[alg]["TotalSleepBlock"].mean(), results[alg]["TotalSleepBlock"].std()))
        values["PercentSleep"].append(
            print_format % (100. * results[alg]["PercentSleep"].mean(), 100. * results[alg]["PercentSleep"].std()))
        values["PercentSleepBlock"].append(print_format % (
        100. * results[alg]["PercentSleepBlock"].mean(), 100. * results[alg]["PercentSleepBlock"].std()))

        values["Accuracy"].append(
            print_format % (100. * results[alg]["AccAlg"].mean(), 100. * results[alg]["AccAlg"].std()))
        values["AccuracyBlock"].append(
            print_format % (100. * results[alg]["AccBlock"].mean(), 100. * results[alg]["AccBlock"].std()))
        values["Precision"].append(
            print_format % (100. * results[alg]["PrecAlg"].mean(), 100. * results[alg]["PrecAlg"].std()))
        values["PrecisionBlock"].append(
            print_format % (100. * results[alg]["PrecBlock"].mean(), 100. * results[alg]["PrecBlock"].std()))
        values["Recall"].append(
            print_format % (100. * results[alg]["RecAlg"].mean(), 100. * results[alg]["RecAlg"].std()))
        values["RecallBlock"].append(
            print_format % (100. * results[alg]["RecBlock"].mean(), 100. * results[alg]["RecBlock"].std()))
        values["F1"].append(print_format % (100. * results[alg]["F1Alg"].mean(), 100. * results[alg]["F1Alg"].std()))
        values["F1Block"].append(
            print_format % (100. * results[alg]["F1Block"].mean(), 100. * results[alg]["F1Block"].std()))
        values["~F1"].append(print_format % (100. * results[alg]["~F1Alg"].mean(), 100. * results[alg]["~F1Alg"].std()))
        values["~F1Block"].append(
            print_format % (100. * results[alg]["~F1Block"].mean(), 100. * results[alg]["~F1Block"].std()))
        values["Specificity"].append(
            print_format % (100. * results[alg]["SpecAlg"].mean(), 100. * results[alg]["SpecAlg"].std()))
        values["SpecificityBlock"].append(
            print_format % (100. * results[alg]["SpecBlock"].mean(), 100. * results[alg]["SpecBlock"].std()))

        values["DeltaStartBlock"].append(
            "%s +- %s" % (results[alg]["DeltaStart"].mean().seconds, results[alg]["DeltaStart"].std().seconds))
        values["DeltaEndBlock"].append(
            "%s +- %s" % (results[alg]["DeltaEnd"].mean().seconds, results[alg]["DeltaEnd"].std().seconds))
...
def annotateSleep(d):
    ...
    d["_cumsleep_diff"] = (d["_noActivity"].cumsum().where(d["_sleepStarts"], np.nan) - 1.).fillna(method="pad").fillna(
        0.0)
...
def annotateAwake(d):
    ...
    d["_cumawake_diff"] = (d["_activity"].cumsum().where(d["_awakeStarts"], np.nan) - 1.).fillna(method="pad").fillna(
        0.0)
...
def sazonov2(df):
    ...
    sazonov = 1.99604 - 0.1945 * df["_w0"] - 0.09746 * df["_w1"] - 0.09975 * df["_w2"] - 0.10194 * df["_w3"] - 0.08917 * \
              df["_w4"] - 0.08108 * df["_w5"] - 0.07494 * df["_w6"] - 0.07300 * df["_w7"] - 0.10207 * df["_w8"]
...
def kripke(df, scaler=0.204):
    ...
    kripke = scaler * (
                0.0064 * df["_a-10"] + 0.0074 * df["_a-9"] + 0.0112 * df["_a-8"] + 0.0112 * df["_a-7"] + 0.0118 * df[
            "_a-6"] + 0.0118 * df["_a-5"] + 0.0128 * df["_a-4"] + 0.0188 * df["_a-3"] + 0.0280 * df["_a-2"] + 0.0664 *
                df["_a-1"] + 0.0300 * df["activity"] + 0.0112 * df["_a+1"] + 0.0100 * df["_a+2"])
...
def sadeh(df, min_value=0):
    ...
    df["_nat"] = ((df["actValue"] >= 50) & (df["actValue"] < 100)).rolling(window=window_nat, center=True,
                                                                           min_periods=1).sum()
...
def tudor_locke(s, min_minutes_sleep=5, min_minutes_awake=10):
    ...
    ****
    ...
    awaketime = (~s.astype(bool)).rolling(window=min_minutes_awake, center=False,
                                          min_periods=1).sum() == min_minutes_awake
...
def webster(df):
    ...
    webster = p * (
                w_m4 * df["_A-4"] + w_m3 * df["_A-3"] + w_m2 * df["_A-2"] + w_m1 * df["_A-1"] + w_0 * df["_A0"] + w_p1 *
                df["_A+1"] + w_p2 * df["_A+2"])
...
def cole(df):
    ...
    cole = p * (w_m4 * df["_A-4"] + w_m3 * df["_A-3"] + w_m2 * df["_A-2"] + w_m1 * df["_A-1"] + w_0 * df["_A0"] + w_p1 *
                df["_A+1"] + w_p2 * df["_A+2"])
...
def get_features(df, winsize=20):
    ...
    for winsize in range(1, winsize):
        df["_mean_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False, min_periods=1).mean().fillna(
            0.0)
        df["_mean_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                     min_periods=1).mean().fillna(0.0)

        df["_median_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False,
                                                              min_periods=1).median().fillna(0.0)
        df["_median_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                       min_periods=1).median().fillna(0.0)

        df["_std_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False, min_periods=1).std().fillna(
            0.0)
        df["_std_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                    min_periods=1).std().fillna(0.0)

        df["_max_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False, min_periods=1).max().fillna(
            0.0)
        df["_max_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                    min_periods=1).max().fillna(0.0)

        df["_min_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False, min_periods=1).min().fillna(
            0.0)
        df["_min_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                    min_periods=1).min().fillna(0.0)

        df["_var_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False, min_periods=1).var().fillna(
            0.0)
        df["_var_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                    min_periods=1).var().fillna(0.0)

        df["_nat_%d" % (winsize)] = ((df["actValue"] >= 50) & (df["actValue"] < 100)).rolling(window=winsize,
                                                                                              center=False,
                                                                                              min_periods=1).sum().fillna(
            0.0)
        df["_nat_centered_%d" % (winsize)] = ((df["actValue"] >= 50) & (df["actValue"] < 100)).rolling(window=winsize,
                                                                                                       center=True,
                                                                                                       min_periods=1).sum().fillna(
            0.0)

        df["_anyact_%d" % (winsize)] = (df["actValue"] > 0).rolling(window=winsize, center=False,
                                                                    min_periods=1).sum().fillna(0.0)
        df["_anyact_centered_%d" % (winsize)] = (df["actValue"] > 0).rolling(window=winsize, center=True,
                                                                             min_periods=1).sum().fillna(0.0)

        if winsize > 3:
            df["_skew_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False,
                                                                min_periods=1).skew().fillna(0.0)
            df["_skew_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                         min_periods=1).skew().fillna(0.0)
            #
            df["_kurt_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=False,
                                                                min_periods=1).kurt().fillna(0.0)
            df["_kurt_centered_%d" % (winsize)] = df["actValue"].rolling(window=winsize, center=True,
                                                                         min_periods=1).kurt().fillna(0.0)
...
def resave_dftest(task):
    ...
    dftest[["mesaid", "linetime", "marker", "interval", "binterval", "gt", "gt_sleep_block", "wake"]].to_csv(dfoutname,
                                                                                                             index=False)
    print("...Done...")
